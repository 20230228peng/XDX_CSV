...
from Src.NodeGraph.node import NodeWire
from Src.NodeGraph.constants import (GRAPH_BACKGROUND_COLOR, SOCKET_OUTPUT,
...
class NodeGraphBase(wx.ScrolledCanvas):
    def __init__(self, parent, registry, config, *args, **kwds):
        ...
        self.Bind(wx.EVT_LEFT_DCLICK, self.OnLeftDoubleDown)
    ...
    def OnLeftDoubleDown(self, event):
        """
        TODO 左键双击事件：获取双击位置对应的节点，并输出节点ID。 20260423
        将数据存入数据库方便核对
        后续可扩展为显示计算结果界面。
        :param event: 鼠标双击事件
        """
        # 获取鼠标位置并转换为场景坐标（考虑滚动和缩放）
        pnt = event.GetPosition()
        winpnt = self.CalcMouseCoords(pnt)

        # 使用已有的 HitTest 方法检测是否点击到节点
        node = self.HitTest(winpnt)

        if node is not None:
            # 获取节点ID
            node_id = node.id
            print(f"双击节点，节点ID: {node_id}")
            print(node.idname)
            print(node.label)
            # 输出节点做特殊处理 node.is_output
            # TODO: 根据节点ID获取计算结果并显示（后续扩展）
            # 例如：self.ShowResultDialog(node)
        else:
            # 步骤一，调取连线数据
            # 步骤二，保存连线数据
            # 步骤三，执行计算任务
            # 步骤四，写入计算结果到output文件夹
            #
            print("双击空白区域，开始执行计算任务")
    ...
    def OnLeftDown(self, event):
        ...
        if self.src_node is not None:
            ...
            # Handle sockets and wires - 传递 event 参数以支持属性编辑
            self.src_socket = self.src_node.HitTest(winpnt,event)
